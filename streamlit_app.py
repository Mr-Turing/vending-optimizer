...
def optimize_packing(inventory_df, drawer_db_df):
    ...
    for index, row in inventory_df.iterrows():
        ...
        # Record Result with requested column names
        if best_drawer_id:
            item_results.append({
                ...
                # Hidden helper cols for calculation
    ...
    # Updated Groupby with new column names
    ...
    # Clean up results_df for display (Remove helper column)
    display_df = results_df.drop(columns=['_drawer_height'])
    
    return display_df, summary_df, total_cabinet_height
